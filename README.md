...
| :----: | :----: | --- |
| `--device /dev/dri` | Obligatorio si se usa VAAPI | Necesario para habilitar la aceleración por hardware mediante VAAPI. |
...
| `-e BOT_TOKEN` | Obligatorio si se desean notificaciones | El token de tu bot de Telegram para enviar notificaciones. |
| `-e CHAT_ID` | Obligatorio si se desean notificaciones | El ID del chat donde se enviarán las notificaciones de Telegram. |
...
| `-e QUALITY=22` | Opcional | Define manualmente el nivel de calidad para la transcodificación, usado en NVENC, VAAPI y QSV. |

...
# Valores por defecto para preset, QP (para VAAPI) y CQ (para NVENC/QSV)
...
process_directory() {
  ...
  local processed_any_file=false  # Para rastrear si se pudo procesar al menos un archivo
    ...
    if [ -f "$item" ] && [[ "$item" == *.mkv || "$item" == *.mp4 || "$item" == *.avi || "$item" == *.mov || "$item" == *.mpeg ]]; then
      ...
      if [[ "$item" != *"_HEVC"* ]] && ! is_file_failed "$item" && ! is_hevc "$item"; then
        ...
        if [ ! -f "$output_file" ]; then
          ...
          if (( available_space >= input_size )); then
            # Intentar procesar el archivo
            if is_file_ready "$item"; then
              transcode_file "$item" "$output_file"

              # Si el archivo fue procesado correctamente, marcar que se procesó al menos uno
              if [ $? -eq 0 ]; then
                processed_any_file=true
              fi
            fi
          else
            # Marcar que el espacio era insuficiente al menos para un archivo
            space_checked=true
          fi
        fi
      fi
  ...
  if [ "$processed_any_file" = false ] && [ "$space_checked" = true ]; then
    echo "Error: No hay suficiente espacio disponible en $OUTPUT_DIR para procesar los archivos pendientes" >> "$LOG_FILE"
    echo "-------------------------------" >> "$LOG_FILE"
    send_telegram_notification "Error: No hay suficiente espacio disponible en $OUTPUT_DIR para procesar los archivos pendientes."
  fi
...
# Inicio
echo "Buscando archivos... - $(date)" >> "$LOG_FILE"
echo " " >> "$LOG_FILE"
